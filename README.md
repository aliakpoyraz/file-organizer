...
The formula lives in [`Formula/fo.rb`](Formula/fo.rb); publishing it needs a
tagged release and a `homebrew-fo` tap repo (see the comments in that file).

### pipx / pip from git

```bash
pipx install "git+https://github.com/aliakpoyraz/file-organizer.git"
# or
python3 -m pip install --user "git+https://github.com/aliakpoyraz/file-organizer.git"
```
